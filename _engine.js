 ...
 * SDK-only approach: all pools must be SDK-supported
...
function filterUsablePools(pools, log = false) {
  ...
  if (!sdkReady) {
    console.warn('[engine] WARNING: SDK not ready, cannot filter pools');
    return [];
  }

  const sdkAvailable = (sdk.getAvailable && sdk.getAvailable()) || { dlmm: false, whirlpool: false, clmm: false };

  const filtered = { dlmm: 0, whirlpool: 0, clmm: 0, cpmm: 0, unsupported: 0 };
  ...
  const usable = pools.filter(pool => {
    ...
    // CPMM: uses math simulation, always usable
    if (type === 'cpmm') {
      return true;
    }
    ...
    // DLMM: requires SDK
    if (type === 'dlmm') {
      if (sdkAvailable.dlmm) return true;
      filtered.dlmm++;
      return false;
    }

    // Whirlpool: requires SDK
    if (type === 'whirlpool') {
      if (sdkAvailable.whirlpool) return true;
      filtered.whirlpool++;
      return false;
    }
    ...
    // CLMM: requires SDK
    if (type === 'clmm') {
      if (sdkAvailable.clmm) return true;
      filtered.clmm++;
      return false;
    }
