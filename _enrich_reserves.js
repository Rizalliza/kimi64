 ...
 * 
 * Usage:
 *  node _enrich_reserves.js pool_orca_only.json --output=pool_orca_only_enriched.json --sdk  
 * 
 * --output=pools_enriched_sdk.json --output= --force-refresh
 *  
 *
 *   node _enrich_reserves.js pool_clmmRay_only_enriched.json --output=pools_validated.json 
 *
 * Example usage:
...
node _enrich_reserves.js ./pools_canonical.json --output=./pools_meta_test.json --rpcUrl=https://solana-mainnet.g.alchemy.com/v2/D45KPIYJAK973XyrdTmjy --force-fresh
...
const pools = require('./pools_meta.json');
...
async function enrichReserves(pools, connection, options = {}) {
  ...
  if (pubkeyList.length === 0) {
    if (log) console.log('[enricher] No vaults to fetch');
    return pools;
  }
...
function validatePools(pools, options = {}) {
  ...
  if (log) {
    ...
    if (issues.noReserves > 0) console.log(`  - No reserves (CPMM/DLMM): ${issues.noReserves}`);
  }
